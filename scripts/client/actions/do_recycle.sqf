...
if ((toLower _type) in KPLIB_o_allVeh_classes) then {
    ...
    private _objectinfo = ((light_vehicles + recon_vehicles + heavy_vehicles + rotarylogi_vehicles + rotarycas_vehicles + fixedwing_vehicles + static_vehicles + support_vehicles + buildings) select {_type == (_x select 0)}) select 0;
