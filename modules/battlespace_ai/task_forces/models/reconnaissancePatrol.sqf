BATTLESPACE_TASK_FORCE_RECON_BUILD_ASSIGNMENT = {
    params ["_sector"];
    ...
    private _position = _center getPos [450 + random 350, _direction - 55 + random 110];
...
BATTLESPACE_TASK_FORCE_RECON_SELECT_LEG = {
    ...
    for "_attempt" from 1 to 8 do {
        private _candidate = _center getPos [750 + random 850, _direction - 90 + random 180];
...
[
    ...
    createHashMapFromArray [
        ["buildAssignment", BATTLESPACE_TASK_FORCE_RECON_BUILD_ASSIGNMENT],
        ...
        [
            "onDecisionTick",
            {
                ...
                private _assigned = !isNil "_operation"
                    ...
                    && {(_operation getOrDefault ["defenseRole", ""]) == "RECON_SCREEN"};
                ...
                if (!_assigned) exitWith {
                    ...
                    if (_destination isEqualTo [] || {_currentLocation distance2D _destination <= 40}) then {
                        [_taskForceName, _taskForce] call BATTLESPACE_TASK_FORCE_RECON_SELECT_LEG;
                    };
                ...
                if (_phase == "DEPLOYING") exitWith {
                    ...
                    if (_currentLocation distance2D _targetPosition <= _arrivalRadius) then {
                        ...
                        [format ["Recon screen %1 began screening ahead of %2", _taskForceName, _assignedSector]] call BATTLESPACE_STRATEGIC_LOG;
                    } else {
                        if (_activeGroups isEqualTo []) then {
                            [_taskForceName, _taskForce] call BATTLESPACE_TASK_FORCE_MOVE_SIMULATED_GROUP;
                        };
