...
BATTLESPACE_STRATEGIC_OFFMAP_REGEN_INTERVAL = 1800;
BATTLESPACE_STRATEGIC_OFFMAP_REGEN_RATIO = 0.05;
BATTLESPACE_STRATEGIC_INITIAL_DELAY = 300;
// Infrastructure evaluation. Reconnaissance shares the ground-allocation cadence.
BATTLESPACE_STRATEGIC_DECISION_INTERVAL = 1800;
BATTLESPACE_STRATEGIC_LOGISTICS_DECISION_INTERVAL = 60;
...
// Battlegroups pursue reported contacts; without contacts they stage, then attack
// an objective after 30 minutes without a player sighting. Paid creation shares
// the ground allowance and BATTLESPACE_STRATEGIC_DEFENDER_DECISION_INTERVAL.
...
BATTLESPACE_OFFENSIVE_QUIET_ATTACK_DELAY = 1800;
BATTLESPACE_OFFENSIVE_RETARGET_DISTANCE = 150;
BATTLESPACE_OFFENSIVE_RETARGET_INTERVAL = 30;
BATTLESPACE_OFFENSIVE_FORMATIONS_PER_TICK = 2; // Leave shared allocation room for reconnaissance and defenders.
...
// Desired local combat strength: source stock interpolates from 1:1 to 1.5:1.
// Existing forces in the area or on their way are subtracted before committing.
BATTLESPACE_OFFENSIVE_RESPONSE_RATIOS = [1.0, 1.5];
BATTLESPACE_OFFENSIVE_MIN_RESPONSE_MANPOWER = 4;
...
// fall back to a fully paid infantry force.
...
// Physical convoys leave catch-up headroom below their slowest vehicle's top speed.
BATTLESPACE_CONVOY_CRUISE_SPEED = 45; // km/h; further limited by the slowest vehicle.
BATTLESPACE_CONVOY_SPACING = 30; // Metres between vehicle centres along the route.
...
BATTLESPACE_AIRLIFT_CHANCE = 0.35; // Deployment mix for new battlegroups and preformed reserves.
BATTLESPACE_AIRLIFT_MAX_RANGE = 20000;
BATTLESPACE_AIRLIFT_LZ_RADIUS = 500;
BATTLESPACE_AIRLIFT_FLIGHT_HEIGHT = 120;
BATTLESPACE_AIRLIFT_LEG_TIMEOUT = 900; // At most three landing attempts, then abort/RTB.
BATTLESPACE_STRATEGIC_DEEP_RECON_MANPOWER = 7;
BATTLESPACE_STRATEGIC_DEEP_RECON_TARGET = 12; // Desired presence within the shared ground allowance.
BATTLESPACE_STRATEGIC_DEEP_RECON_PER_TICK = 2;
BATTLESPACE_STRATEGIC_DEEP_RECON_COOLDOWN = 600;
BATTLESPACE_STRATEGIC_DEEP_RECON_RETURN_FIRE_DURATION = 120; // Return to silent observation after this long without incoming fire.
...
BATTLESPACE_AIR_BOMB_HEIGHT = 1500; // Metres above the highest terrain on the planned run
BATTLESPACE_AIR_JET_ATTACK_HEIGHT = 450;
BATTLESPACE_AIR_HELI_ATTACK_HEIGHT = 180;
BATTLESPACE_AIR_STANDOFF_MAX = 4500; // Still limited by the loaded weapon's native range
BATTLESPACE_AIR_ENGAGEMENT_KEEP_RANGE = 8000; // Physical attack/egress retention, not initial activation
...
// Construction needs an arrived, surviving objective garrison. Virtual arrivals count.
BATTLESPACE_STRATEGIC_FORTIFICATION_GARRISON_RADIUS = 350;
// Earthworks follow slopes; weapons cap at 15 degrees and bunker floors remain level.
BATTLESPACE_STRATEGIC_FORTIFICATION_MAX_SLOPE = 25;
