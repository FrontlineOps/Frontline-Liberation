...
if (!_assigned && {!(combatMode _group in ["YELLOW", "RED"])}) exitWith {};
...
if (_visible || {_nearMiss}) then {
    if (behaviour _unit != "COMBAT") then {_group setBehaviour "COMBAT"};
} else {
    if (behaviour _unit in ["SAFE", "CARELESS", "STEALTH"]) then {_group setBehaviour "AWARE"};
};
if (_visible) then {
