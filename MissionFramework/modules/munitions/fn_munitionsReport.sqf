...
_rows pushBack format ["GLOBAL PARTICLE CAP: server reserved/live slots=%1; this owner awaiting grants=%2. Reservations include queued work; returns are batched after native deletion. No timeout reclaim of live owners.", ["server only", localNamespace getVariable ["KPLIB_munitionsBudgetUsed", 0]] select isServer, count (localNamespace getVariable ["KPLIB_munitionsBudgetPending", createHashMap])];
