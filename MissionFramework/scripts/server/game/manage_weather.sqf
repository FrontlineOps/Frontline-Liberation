...
private _newWeather = selectRandom (_weatherSets select (_mode - 1));
...
while {GRLIB_endgame == 0} do {
    ...
    _newWeather = selectRandom (_weatherSets select (_mode - 1));
    ...
    private _next = time + 3000;
    // Reuse this worker; a live mode edit schedules a smooth new transition.
    waitUntil {sleep 5; GRLIB_endgame != 0 || {time >= _next} || {GRLIB_weather_param != _mode}};
};
