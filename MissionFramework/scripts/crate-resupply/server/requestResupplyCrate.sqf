requestResupplyCrate = {
    params [["_requester", objNull, [objNull]], ["_crateName", "", [""]]];
    if !([_requester, "spawn", objNull, _crateName] call KPLIB_fnc_validateResupplyRequest) exitWith {};
    ...
    (localNamespace getVariable "KPLIB_resupplyRegistry") set [netId _crate, [_crateName, _squadName, typeOf _crate]];
    ...
    if(_isSpecialCrate) then {
        ...
        format ["Mission Time is %1, setting Reset to %2", _missionTime, _missionTime + (localNamespace getVariable "KPLIB_ResupplyDefaultSpecialtyCooldown")] call resupplyLog;
    ...
    localNamespace setVariable [_squadName, _currentAllocations];

    missionNamespace setVariable [_squadName, _currentAllocations, true];

    [_squadName, _crateName] call (localNamespace getVariable "KPLIB_startCrateCooldown");
