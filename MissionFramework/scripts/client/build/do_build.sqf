...
while {true} do {
    ...
    if !(_selectedBuildType in [2, 3, 4, 5, 6, 7, 99]) then {
        dobuild = 0;
        build_confirmed = 0;
        continue;
    };
    ...
    if (build_confirmed isEqualTo 2 && {!isNull _preview}) then {
        ...
        if (_isFobBuild) then {
            ...
            [_selectedBuildType, _selectedBuildIndex, _placedPosition, _vehicleDirection, _supportsVector && {KP_vector}] remoteExecCall ["build_remote_call", 2];
        };
    };
