...
private _itemList = KP_liberation_allowed_items;


// Check single item slots
if !(toLower (headgear player) in _itemList) then {
    _removedItems pushBack (headgear player);
    removeHeadgear player;
};
if !(toLower (goggles player) in _itemList) then {
    _removedItems pushBack (goggles player);
    removeGoggles player;
};
if !(toLower (uniform player) in _itemList) then {
    _removedItems pushBack (uniform player);
    removeUniform player;
};
if !(toLower (vest player) in _itemList) then {
    _removedItems pushBack (vest player);
    removeVest player;
};
if (!(toLower (backpack player) in _itemList) && ((backpack player) != _backpack)) then {
...
} forEach (((_items arrayIntersect _items) - _itemList) select {!([_x] call KPLIB_fnc_isRadio)});
...
} forEach ((_items arrayIntersect _items) - _itemList);
